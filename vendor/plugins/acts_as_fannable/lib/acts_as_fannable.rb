  ...
  module Acts
    module Fannable 
      ...
      module InstanceMethods
        ...
        def is_fan?(user)
         fan = Fan.find(:all,
            :conditions => ["user_id = ? and fannable_type = ? and fannable_id = ?",user.try(:id), self.class.name,self.try(:id)]
          )
          !(fan.blank?)
        end

        def fan(user)
         fan = Fan.find_or_initialize_by_user_id_and_fannable_type_and_fannable_id(user.try(:id), self.class.name, self.try(:id))
        end
